...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const deleted = await clearMailbox({ refreshToken, clientId, email, mailbox: 'Junk' })
    return res.status(200).json({ message: 'Junk mailbox processed successfully', deleted })
  } catch (error) {
    return sendHandlerError(res, error, '清空垃圾箱失败')
  }
}
