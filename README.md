...
Microsoft OAuth2 邮件 API 与本地邮箱工作台。当前版本：**0.6.6**。
...
```text
邮箱----密码或Key----Client ID----Refresh Token----备注（可选）
```

- 默认分隔符为 `----`，导入时可以自定义。分隔符不能出现在邮箱、密码、Client ID、Refresh Token 中，否则字段会被切错；备注是最后一个字段，内部可以包含分隔符。
- 支持粘贴文本、选择文件或选择文件夹。
- 同一邮箱地址不会重复导入。
- 导入验证失败后可一键复制失败账号，修改后可直接重新导入。
- 支持搜索、分页、编辑（包括备注）、单个/批量删除和复制导出。
- 备注为可选字段，最长 2000 字符，仅保存在本地；只修改备注时不会重新请求 Microsoft 验证凭证。
- 编辑弹窗中 Client ID 与 Refresh Token 并排显示为同款文本框，备注独占一行，三者都可以拖动右下角自由调整大小。
- 账号、密码、Client ID、Refresh Token 和备注均支持点击复制；长内容只显示前后片段。备注显示多少由当前备注列宽决定，宽屏能看到更多内容，手机保持紧凑；汉字按两个字符宽计算，中英文备注的视觉长度一致。
- 复制导出的格式与导入一致，有备注时在行尾追加 `----备注`（备注为空则不追加），导出的文本可以直接重新导入。
- 点击收件箱会合并加载收件箱和垃圾箱，垃圾箱邮件会显示“垃圾箱”标记。
- 支持刷新当前邮箱以及查看邮件详情。邮件列表和邮件详情里的发件人、收件人都可以点击复制；在列表里点地址只复制，不会顺带打开这封邮件。
